...
--
--   Una sola migración: 083_zona_del_jugador.sql
--
--   QUÉ HACE
--     · `zona_del_jugador(uuid)` — la ciudad donde más juega alguien,
--       deducida de los torneos a los que se ha inscrito. Sin preguntarle
--       nada: `users` no tiene ciudad y no hace falta que la tenga.
--     · `buscar_pareja` pasa a ordenar por zona: los de tu ciudad primero.
--       Devuelve dos columnas nuevas (`zona`, `misma_zona`), así que se tira
--       y se vuelve a crear — `create or replace` no puede cambiar el tipo de
--       retorno de una función que devuelve tabla.
--
--   ES IDEMPOTENTE: se puede correr dos veces sin romper nada.
--
--   COMPROBACIÓN, ya autenticado y desde la app:
--     select zona, misma_zona, full_name from public.buscar_pareja('quinta');
-- ════════════════════════════════════════════════════════════════════
