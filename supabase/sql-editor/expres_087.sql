-- ════════════════════════════════════════════════════════════════════
-- RALLY · 087 — ESTA ES LA QUE FALTABA DE VERDAD
--
--   La 085 que corriste se ejecutó bien pero sobre el nombre equivocado:
--   parcheó `sembrar_expres` cuando la función viva se llama
--   `sortear_expres` desde la migración 078. `create or replace` sobre un
--   nombre que ya no existe no avisa — crea una función nueva. Por eso
--   dijo "Success" y el sorteo siguió sin cerrar nada.
--
--   Esta borra la huérfana, aplica el cambio donde tocaba y repara el
--   torneo que ya sorteaste.
--
--   Es idempotente. Cópiala entera.
-- ════════════════════════════════════════════════════════════════════

-- 087_sortear_expres_cierra_de_verdad.sql  ·  RALLY
--
-- LA 085 PARCHEÓ UNA FUNCIÓN QUE NO LLAMA NADIE
--
-- ► QUÉ PASÓ
--   La 077 creó la función como `sembrar_expres`. La 078 la RENOMBRÓ a
--   `sortear_expres` —porque en un exprés los grupos se sortean, no se
--   siembran— y es ese nombre el que invoca la Edge Function.
--
--   La 085 hizo `create or replace function public.sembrar_expres(...)`. Con
--   el nombre viejo. Y `create or replace` sobre un nombre que ya no existe no
--   avisa de nada: CREA UNA FUNCIÓN NUEVA. Así que la base acabó con dos —
--   `sortear_expres`, la viva, con el cuerpo de la 077; y `sembrar_expres`,
--   recién creada y que no llama nadie.
--
--   Por eso el SQL Editor dijo "Success. No rows returned" y el sorteo
--   siguiente volvió a dejar la categoría en 'open': se ejecutó correctamente
--   un cambio sobre código muerto.
--
-- ► QUÉ HACE ESTA
--   1. Borra la `sembrar_expres` huérfana que creó la 085.
--   2. Aplica el cambio donde tocaba: sobre `sortear_expres`.
--   3. Repara los exprés que ya se sortearon con el cuerpo viejo (misma
--      reparación de la 086, que es idempotente y hay que volver a pasar
--      porque desde entonces se sorteó otro).
--
--   Es idempotente entera. Correrla dos veces no hace nada la segunda.

begin;

-- ── 1. La huérfana ────────────────────────────────────────────────────────
--    `if exists` porque en una base limpia nunca llegó a crearse.
drop function if exists public.sembrar_expres(uuid, uuid, jsonb, jsonb);

-- ── 2. El cambio, sobre la función de verdad ──────────────────────────────

create or replace function public.sortear_expres(
  p_actor       uuid,
  p_category_id uuid,
...
revoke all on function public.sortear_expres(uuid,uuid,jsonb,jsonb)
...
comment on function public.sortear_expres(uuid,uuid,jsonb,jsonb) is
  'Sortea un exprés entero —grupos, reparto, tabla a cero y calendario—, '
  ...
  'largo. Se sortea una vez: expres_config.sorteado_at es el candado.';

-- ── 3. Reparar lo ya sorteado con el cuerpo viejo ─────────────────────────
--    Mismo criterio que la 086: solo exprés, ya sorteado y con grupos de
--    verdad. Un exprés sin sortear NO se toca — ahí la inscripción tiene que
--    seguir abierta.
...
-- COMPROBACIÓN
--
--   -- a) ya no queda la huérfana, y la viva tiene el cierre dentro:
--   select p.proname,
--          position('categorias_abiertas' in pg_get_functiondef(p.oid)) > 0
--            as cierra_la_categoria
--     from pg_proc p
--    where p.proname in ('sembrar_expres', 'sortear_expres');
--
--   -- Esperado: UNA sola fila, sortear_expres, con true.
--
--   -- b) el torneo ya sorteado quedó cerrado:
--   select t.name, t.status as torneo, c.status as categoria
--     from public.tournaments t
