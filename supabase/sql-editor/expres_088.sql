...
--
--   Una sola migración: 088_expres_formato_se_rellena.sql
--
--   QUÉ DESBLOQUEA
--     Terminada la fase de grupos de un exprés no había forma de armar el
--     cuadro. Y no era solo el botón: `generate-bracket` no escribe la
--     columna `formato`, y el trigger de la 075 rechaza cualquier partido
--     de exprés que llegue sin ella. O sea que el botón habría fallado.
--
--     Ahora el trigger la RELLENA desde `expres_etapa` en vez de
--     rechazar. Sigue rechazando si la etapa no está configurada, que es
--     el único caso donde de verdad no hay nada que leer.
--
--   ES un `create or replace` de una función. Idempotente.
--
--   DESPUÉS: en el panel del torneo exprés aparece "Armar cuartos de
--   final", apagado hasta que estén los 40 partidos de grupo.
-- ════════════════════════════════════════════════════════════════════
